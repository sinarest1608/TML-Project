...
if __name__ == "__main__":
    ...
    train_x = x_train[:samples_to_draw]
    train_y = y_train[:samples_to_draw]
    test_x = x_test[:samples_to_draw]
    test_y = y_test[:samples_to_draw]
    
    num_classes = np.unique(y_train).shape[0]
    assert num_classes == 100 # cifar10
    
    ### load the target model (the one we want to protect)
    target_model_fp = './cifar100_cnn_new.h5'
    ...
    train_acc = np.mean(train_y[:,0] == np.argmax(pred_y, axis=-1))
    
    pred_y = predict_fn(test_x)
    # test_acc = np.mean(np.argmax(test_y, axis=-1) == np.argmax(pred_y, axis=-1))
    test_acc = np.mean(test_y[:,0] == np.argmax(pred_y, axis=-1))
...
#         print('{} --- Attack accuracy: {:.2f}%; advantage: {:.3f}; precision: {:.3f}; recall: {:.3f}; f1: {:.3f}'.format(attack_str, attack_acc*100, attack_adv, attack_precision, attack_recall, attack_f1))
    ...
    print('\n------------ Adversarial Examples ----------')
    advexp_fps = []
    advexp_fps.append(('Adversarial examples attack0', 'cifar100_adversary.npz'))
    ...
    for i, tup in enumerate(advexp_fps):
        ...
        benign_acc = np.mean(benign_y[:,0] == np.argmax(benign_pred_y, axis=-1))
        
        adv_pred_y = predict_fn(adv_x)
        #print(benign_y[0:10], adv_pred_y[0:10])
        adv_acc = np.mean(benign_y[:,0] == np.argmax(adv_pred_y, axis=-1))
